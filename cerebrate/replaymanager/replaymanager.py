...
class ReplayManager:
    def __init__(self, app_data_path: str):
        ...
        replay_archive_path = os.path.join(
            app_data_path, REPLAY_ARCHIVE_SUBDIRECTORY_NAME
        )
    ...
    def tag_replay(self, replay_path: str, tags: list):
        ...
        replay_updated = self.__db.update(
            _add_tags(tags), tinydb.Query()["hash"] == replay_hash
        )
        if not replay_updated:
            replay_canonical_path = os.path.join(
                self.__replay_archive_path, replay_hash + ".SC2Replay"
            )
            shutil.copyfile(replay_path, replay_canonical_path)
            self.__db.insert(
                {
                    "hash": replay_hash,
                    "canonical_path": replay_canonical_path,
                    "tags": tags,
                }
            )
    ...
    def query_replays(
        ...
        query = (
            query & replay["hash"].test(lambda x: x in replay_hashes)
            if replay_hashes
            else query
        )
    ...
    def tag_frequency(self, source_replays: list) -> list:
        if source_replays:
            ...
            matches = self.__db.search(
                tinydb.Query()["hash"].test(lambda x: x in replay_hashes)
            )
        else:
